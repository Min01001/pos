...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['submit'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO incomes (note, income, date) VALUES (?, ?, ?)");
...
<div class="content-page">
    <div class="container-fluid mt-6">
            ...
            <div class="col-md-6">
                <div class="bg-grey text-dark p-3">
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post"
                        ...
                        <div class="form-group row mb-3">
                            <label for="incomeInput" class="col-sm-3 col-form-label text-white">ဝင်ငွေ</label>
                            <div class="col-sm-9">
        ...
        $incomesql = "SELECT SUM(income) AS total_income FROM incomes";
        $incomeresult = $conn->query($incomesql);
        $resultincome = $incomeresult->fetch_assoc();
        echo $resultincome['total_income'] . " ကျပ်";
        ...
        ?>
                ...
                if ($searchQuery) {
                    $sql = "SELECT id, note, income, date FROM incomes WHERE note LIKE ? ORDER BY id DESC";
                    $stmt = $conn->prepare($sql);
                    $searchParam = "%" . $searchQuery . "%";
                    $stmt->bind_param("s", $searchParam);
                } else {
                    $sql = "SELECT id, note, income, date FROM incomes ORDER BY id DESC";
                    $stmt = $conn->prepare($sql);
                }
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>";
                        echo "<form id='form_edit_" . htmlspecialchars($row["id"]) . "' method='POST' action='delete_income.php' onsubmit=\"return confirm('Are you sure you want to Delete?')\">";
